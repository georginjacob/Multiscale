rm(list = ls())
DATA=read.csv(file = "VS_distractor_congruence.csv")
summary(DATA)
...
lmm_RT=lmer(meanRT~Distractor*ShapePairs+(1|Subject),data=DATA, REML = FALSE)
anova(lmm_RT)

lmm_invRT=lmer(invmeanRT~Distractor*ShapePairs+(1|Subject),data=DATA, REML = FALSE)
anova(lmm_invRT)
